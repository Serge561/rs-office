"""Функционал постановки задач для celery."""
...
def send_activate_email_message_task(user_id):
    ...
    2. Отправка письма подтверждения осуществляется через функцию:
...
def send_contact_email_message_task(subject, email, content, ip, user_id):
    ...
    2. Отправка письма из формы обратной связи осуществляется через функцию:
